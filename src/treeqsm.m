...
if size(P,2) > 3    % size(p, 1)返回矩阵行数, size(p, 2)返回矩阵列数
    P = P(:,1:3);   % 取P矩阵的第1-3列的所有数据
end
% Only double precision data
if ~isa(P,'double') % 判断P矩阵数据类型是否为 double 类型
    P = double(P);  % 若不是，则转换为double类型
end
...
for h = 1:nd
  ...
  Time(2) = toc-Time(1);              % 保存tree_sets后的时间
  if inputs.disp == 2
